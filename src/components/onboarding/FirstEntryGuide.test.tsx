...
describe('FirstEntryGuide', () => {
  ...
  it('starts with a functional app preview instead of a decorative icon page', () => {
    ...
    expect(screen.getByRole('heading', { name: '先从一张牌开始' })).toBeInTheDocument();
    expect(screen.getByText('导览路线')).toBeInTheDocument();
    expect(screen.getByText('今天的一张牌')).toBeInTheDocument();
    expect(screen.getByText('复盘和牌义注疏')).toBeInTheDocument();
    ...
    expect(screen.getByRole('button', { name: '看推荐路线' })).toBeInTheDocument();
  });
  ...
  it('moves through feature preview steps with the primary action', async () => {
    ...
    await user.click(screen.getByRole('button', { name: '看推荐路线' }));
    expect(await screen.findByRole('heading', { name: '日运抽牌' })).toBeInTheDocument();
    expect(screen.getByText('日运练习')).toBeInTheDocument();
    expect(screen.getByText('牌义注疏')).toBeInTheDocument();
    expect(screen.getAllByTestId('guide-card-back')).toHaveLength(1);

    await user.click(screen.getByRole('button', { name: '看如何记录' }));
