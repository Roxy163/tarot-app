...
export default function App() {
  ...
  const generatePublicId = (createdAt: string) => {
    ...
    const chars = 'ABCDEFGHJKLMNPQRSTUVWXYZ23456789'; // Excluded 0, O, I, L, 1
    let randomCode = '';
    for (let i = 0; i < 4; i++) {
  ...
  const fetchProfile = async (userId: string) => {
    ...
    if (error && error.code === 'PGRST116') {
      ...
      const newProfile = {
        id: userId,
        display_name: session?.user?.email?.split('@')[0] || '研习阁主',
        bio: '研精覃思，洞见未来',
        createdAt: timestamp,
        user_public_id: generatePublicId(timestamp)
      };
      const { data: created } = await supabase.from('profiles').insert([newProfile]).select().single();
      if (created) setProfile(created);
    } else if (data) {
      if (!data.user_public_id) {
        const publicId = generatePublicId(data.createdAt || new Date().toISOString());
        const { data: updated } = await supabase.from('profiles').update({ user_public_id: publicId }).eq('id', userId).select().single();
  ...
  const checkMigration = () => {
    ...
    try {
      ...
      // If logged in, sync automatically as requested
      if (session) {
        handleMigration(true);
        return;
      }

      // Session-based suppression for non-logged users
      if (window.sessionStorage.getItem('migration_prompt_checked')) return;
      
      const lastAsked = localStorage.getItem('migration_prompt_last_asked');
      const today = new Date().toDateString();
      if (lastAsked === today) return;

      setShowMigrationPrompt(true);
      window.sessionStorage.setItem('migration_prompt_checked', 'true');
    } catch (e) { /* Ignore */ }
  };
  ...
  const handleMigration = async (shouldSync: boolean) => {
    if (shouldSync && session?.user) {
      ...
      try {
        ...
        // If synced, we can clear the guest data
        ...
        setSnackbar({ isOpen: true, message: '✨ 记录已同步至云端，永不遗失。' });
      } catch (e) {
      ...
      // If "No", we don't delete (as requested "only use local"), 
      // but we mark it as asked for today
      localStorage.setItem('migration_prompt_last_asked', new Date().toDateString());
    }
  ...
  const handleLogout = async () => {
    ...
    setProfile(null);
    setActiveTab('home');
    setReadings(INITIAL_READINGS.map(r => ({ ...r, isExample: true })));
    localStorage.removeItem('tarot_guest_data');
    ...
    try {
      ...
      if (editingReading) {
        ...
        const reading: TarotReading = {
          ...
          authorName: profile?.display_name || profile?.nickname || session?.user?.email?.split('@')[0] || '研习阁主',
          ...readingData
        };
    ...
    return (
      <div className="min-h-screen bg-forest-bg flex flex-col items-center justify-center p-4">
        <motion.div 
          ...
          className="text-forest-accent mb-4"
        >
          <Sparkles size={48} />
        </motion.div>
        <p className="text-forest-muted text-sm font-serif animate-pulse">正在执印入阁...</p>
      </div>
    );
  }
    ...
    <div className="min-h-screen bg-forest-bg flex flex-col max-w-4xl mx-auto px-4 py-6 sm:py-8 relative overflow-x-hidden">
        ...
        {isSidebarOpen && (
          <>
            ...
            <motion.aside
              ...
              className="fixed left-0 top-0 bottom-0 w-80 bg-white z-[110] shadow-[10px_0_40px_-10px_rgba(31,54,36,0.3)] border-r border-forest-accent/10 flex flex-col"
            >
              ...
              <div className="relative h-full flex flex-col z-10">
                <div className="p-6 border-b border-forest-accent/5 flex items-center justify-between bg-forest-bg/50 backdrop-blur-md">
              <div className="flex items-center gap-3">
                <div className="w-10 h-10 rounded-full bg-forest-accent flex items-center justify-center text-forest-card font-serif shadow-md">
                  {profile?.display_name?.[0] || profile?.nickname?.[0] || (session ? (session.user?.email?.[0].toUpperCase() || '研') : '访')}
                </div>
                <div className="overflow-hidden">
                  <h3 className="font-serif text-forest-accent font-bold truncate">
                    {session ? `${profile?.display_name || profile?.nickname || session.user?.email?.split('@')[0]}阁主` : '访客 · 观阁中'}
                  </h3>
            ...
            <div className="flex-1 overflow-y-auto p-4 space-y-2">
              ...
              <button 
                onClick={() => {
                  ...
                  setSelectedAuthor(profile?.display_name || profile?.nickname || session.user?.email?.split('@')[0] || '研习阁主');
              ...
              <div className="pt-4">
                ...
                {session ? (
                  <div className="space-y-1">
                    ...
                    >
                      <div className="flex items-center gap-3">
                        <Lock size={18} className="text-forest-accent" />
                    ...
                    <button 
                      ...
                      className="w-full flex items-center gap-3 p-3 rounded-xl hover:bg-forest-accent/5 text-forest-muted hover:text-forest-accent transition-all group"
                    >
                      <LogOut size={18} />
        ...
        {isSecurityModalOpen && (
          ...
          >
            <div className="space-y-6">
                  ...
                  <button 
                    onClick={async () => {
                      ...
                      try {
                        ...
                        setSnackbar({ isOpen: true, message: '✨ 通行密码已成功修定。' });
                      } catch (err: any) {
                        alert(`修定失败: ${err.message}`);
                      } finally {
                        setTimeout(() => setSnackbar(prev => ({ ...prev, isOpen: false })), 3000);
                      }
                    }}
                    className="w-full py-4 bg-forest-accent text-white rounded-xl text-sm font-bold shadow-lg shadow-forest-accent/10 hover:opacity-90 active:scale-95 transition-all"
                  >
                    修订密码
                  </button>
                </div>
              ...
              <div className="p-6 bg-white rounded-2xl border border-forest-border/50">
                <div className="flex items-center gap-3 text-forest-muted font-bold mb-2">
                  <Info size={18} />
                  <h4 className="text-sm">当前通行邮箱</h4>
                </div>
                <p className="text-xs font-mono text-forest-accent bg-forest-accent/5 px-4 py-2 rounded-lg inline-block">
                  {session?.user?.email}
                </p>
              </div>
            </div>
      ...
      <nav className="fixed bottom-0 left-0 right-0 bg-white/90 backdrop-blur-md border-t border-forest-accent/10 z-[100] px-2 py-1 safe-area-inset-bottom shadow-[0_-4px_20px_-10px_rgba(0,0,0,0.1)]">
        <div className="flex justify-around items-center max-w-lg mx-auto h-16">
          ...
          {session ? (
            <TabButton id="tab-profile" active={activeTab === 'profile'} onClick={() => {
              setSelectedAuthor(profile?.display_name || profile?.nickname || session.user?.email?.split('@')[0] || '研习阁主');
              setActiveTab('profile');
            }} icon={User} label="印鉴" />
          ) : (
      ...
      <main className="flex-1 pb-24">
        <AnimatePresence mode="wait">
            ...
            <motion.div key="private" initial={{ opacity: 0, x: -20 }} animate={{ opacity: 1, x: 0 }} exit={{ opacity: 0, x: 20 }} className="space-y-4 sm:space-y-6">
              ...
              {!filteredReadings || filteredReadings.length === 0 ? (
                <div className="text-center py-24 text-forest-muted bg-white/50 rounded-3xl border border-dashed border-forest-accent/20 flex flex-col items-center gap-4">
                  ...
                  <button 
                    onClick={() => { setActiveTab('add'); setSearchQuery(''); setSearchTags([]); }}
          ...
          {!['home', 'add', 'private', 'public', 'profile', 'metadata'].includes(activeTab) && (
            <motion.div 
              key="fallback"
              initial={{ opacity: 0 }}
              animate={{ opacity: 1 }}
              className="flex flex-col items-center justify-center py-20 text-forest-muted"
            >
              <History size={48} className="opacity-20 mb-4" />
              <p>内容正在入阁... 或此典籍已遗失</p>
              <button 
                onClick={() => setActiveTab('home')}
                className="mt-4 text-forest-accent underline font-serif"
              >
                返回研习台
              </button>
            </motion.div>
