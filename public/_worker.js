const DEFAULT_FEEDBACK_EMAIL = 'roxy163@outlook.com';
...
const readProviderMessage = async (response) => {
  try {
    ...
    if (contentType.includes('application/json')) {
      ...
      return [payload?.message, payload?.error, payload?.success, payload?.providerMessage]
...
const handleFeedback = async (request, env) => {
  ...
  const feedbackEmail = env.FEEDBACK_EMAIL || DEFAULT_FEEDBACK_EMAIL;
  const formEndpoint = `https://formsubmit.co/ajax/${feedbackEmail}`;
