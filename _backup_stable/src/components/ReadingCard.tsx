...
export const ReadingCard: React.FC<ReadingCardProps> = ({ 
  ...
  const renderCards = (isMini = false) => {
    ...
    if (isMini) {
      return (
        <div className="h-24 overflow-hidden relative rounded-xl bg-forest-bg/30 p-2">
  ...
  return (
    <div 
      ...
      className={`bg-white rounded-2xl border border-forest-accent/10 hover:border-forest-accent/30 transition-all overflow-hidden cursor-pointer active:scale-[0.99] group/card-main ${isExpanded ? 'p-4 sm:p-6 shadow-xl' : 'p-4 shadow-sm hover:shadow-md'}`}
    >
      <div className="flex justify-between items-start gap-3">
        <div className="flex-1 min-w-0">
          <div className="flex items-center gap-2 text-[10px] text-forest-muted mb-1.5 opacity-80">
            ...
            {reading.isForClient && !isPublicView && (
              <>
                <span className="mx-1 opacity-20">|</span>
                <span className="text-forest-accent font-medium">客户: {reading.clientName}</span>
              </>
            )}
          </div>
          <h3 className={`${isExpanded ? 'text-lg sm:text-xl' : 'text-base'} font-serif text-forest-text leading-snug transition-all`}>
            {reading.question}
            {reading.isExample && (
              <span className="ml-2 px-1.5 py-0.5 bg-forest-accent/10 text-forest-accent rounded text-[9px] font-bold">案例</span>
            )}
          </h3>
          
          {!isExpanded && (
            <div className="mt-3 flex flex-wrap items-center gap-x-2 gap-y-1 text-sm text-forest-ink/70">
              {reading.cards.map((card, idx) => (
                <React.Fragment key={idx}>
                  <span className="font-medium">
                    {card.name}{card.isReversed ? '（逆）' : ''}
                  </span>
                  {idx < reading.cards.length - 1 && <span className="opacity-40 font-bold mx-0.5">·</span>}
                </React.Fragment>
              ))}
            </div>
          )}
        ...
        <div className="flex items-center gap-1 shrink-0 bg-forest-accent/5 p-1 rounded-full border border-forest-accent/10">
          <button 
            onClick={(e) => {
              e.stopPropagation();
              handleShare();
            }}
            className="p-1.5 rounded-full text-forest-accent hover:bg-white transition-colors"
            title="分享"
          >
            <Share2 size={14} />
          </button>
          ...
          <button 
            className="p-1.5 rounded-full text-forest-accent group-hover/card-main:translate-y-0.5 transition-transform"
          >
      ...
      {!isExpanded && (
        <div className="mt-4 flex flex-col gap-4">
          {renderCards(true)}
          <div className="mt-2 pt-2 border-t border-forest-accent/5">
            {renderTags(false)}
          </div>
        </div>
      )}
      ...
      {isExpanded && (
        <div className="mt-6 animate-in fade-in slide-in-from-top-2 duration-300">
          ...
          <div className="flex items-center justify-between mb-2">
            <span className="text-[10px] font-bold text-forest-muted uppercase tracking-wider">布阵图</span>
            <span className="text-[10px] text-forest-accent/60 flex items-center gap-1"><Sparkles size={10} /> 点击牌面查看单牌解读</span>
          </div>
